...
my $charge_max = $capacity *      $max_soc  if defined $capacity;
my $charge_min = $capacity * (1 - $max_dod) if defined $capacity;
    ...
    while ($year < $end_year) {
        ...
        for (my $item = 0; $item < $items; $item++) {
            ...
            if (defined $capacity) { # storage present
                my $capacity_to_fill = $charge_max - $soc;
                ...
                my $dischg_delta = 0;
                my $AC_loss = 0;
                if ($soc > $charge_min) { # storage not empty
                    ...
                    $discharge = $soc - $charge_min
                        if $discharge > $soc - $charge_min;
...
sub save_statistics {
    ...
    while ($days < 365) {
        ...
        $gross   += $gross_across;
        $PV_loss += $PV_net_loss[$month][$day][$hour];
        $net     += $net_across;
        if (!$max) {
            $hload   +=        $load[$month][$day][$hour];
